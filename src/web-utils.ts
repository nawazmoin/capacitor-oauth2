...
export class WebUtils {
    ...
    static getTokenEndpointData(options: WebOptions, code: string): string {
        ...
        for (const key in options.additionalParameters) {
            body += "&" + key + "=" + options.additionalParameters[key];
        }
        return body;
    }
...
export class CryptoUtils {
    ...
    static toBase64(bytes: Uint8Array): string {
        ...
        for (let i = 0; i < len; i += 3) {
            ...
            base64 +=
                this.BASE64_CHARS[((bytes[i] & 3) << 4) | (bytes[i + 1] >> 4)];
            base64 +=
                this.BASE64_CHARS[
    ...
    static deriveChallenge(codeVerifier: string): Promise<string> {
        if (codeVerifier.length < 43 || codeVerifier.length > 128) {
            return Promise.reject(
                new Error("ERR_PKCE_CODE_VERIFIER_INVALID_LENGTH")
            );
        }
        if (!CryptoUtils.HAS_SUBTLE_CRYPTO) {
        ...
        return new Promise((resolve, reject) => {
            crypto.subtle
                .digest("SHA-256", this.toUint8Array(codeVerifier))
                .then(
                    (arrayBuffer) => {
                        return resolve(
                            this.toBase64Url(
                                this.toBase64(new Uint8Array(arrayBuffer))
                            )
                        );
                    },
                    (error) => reject(error)
